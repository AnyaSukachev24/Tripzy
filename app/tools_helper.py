...
def _get_flight_price_metrics(origin: str, destination: str, departure_date: str, currency: str = "USD") -> Dict:
    ...
    # Try using the rate-limited client if available, else local helper
    try:
    ...
    # Fallback to local client if rate limiter fails or not used (though above imports should work)
    if not 'amadeus' in locals() or not amadeus:
        ...
        if amadeus:
            try:
                ...
                if response.data:
                    return {"source": "Amadeus", "metrics": response.data}
            except Exception:
                pass
